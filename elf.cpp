...
u32 LElfFile::Read(LPVOID lpBuffer,u32 dwBytes){
   u32 dwRead,dwLength;
   ...
   if(cache == NULL || dwCacheEnd == 0){
       ...
       if(cache != NULL){
           _stream->Read(cache,16384,&dwRead);
   ...
   _stream->Read(lpBuffer,dwBytes,&dwRead);
   return dwRead;
}

u32 LElfFile::Seek(LONG dwDistanceToMove,u32 dwMoveMethod){
   s64 rel_ofs;
   ...
   switch(dwMoveMethod){
       ...
       case FILE_END:{
		   ...
		   _stream->Tell(&p);
		   _stream->Seek(0,FILE_END);
		   _stream->Tell(&pp);
		   _stream->Seek(p,FILE_BEGIN);
           rel_ofs =pp + dwDistanceToMove;
       }
       break;
       default:
           return (u32)-1;
   ...
   if(rel_ofs < dwCacheStart || rel_ofs >= dwCacheEnd){
       dwCacheEnd = 0;
       dwCachePos = dwCacheStart = (u32)rel_ofs;
       return _stream->Seek(dwDistanceToMove,dwMoveMethod);
   }
//   return LFile::Seek(dwDistanceToMove,dwMoveMethod);
   return (u32)(dwCachePos = (u32)rel_ofs);
}

int LElfFile::Open(){
	int i;
	ELFSectionHeader *h;
	u32 abbrev,total,end,ddata;
	CompileUnit *last,*unit,*comp;
 	ARanges *r;

//   if(FileStream::Open((char *)_name.c_str()))
//       return -1;
   if(header.magic == 0x464C457F && header.clazz == 1 && header.e_machine == 8)
       return 0;
   Seek(0,FILE_BEGIN);
   ...
   if((dbgHeader = GetSectionByName((char *)".debug_info")) == NULL)
		return 0;
   h = GetSectionByName((char *)".debug_abbrev");
   abbrev = ReadSection(h);
   if((h = GetSectionByName((char *)".debug_str")) != NULL)
...
LONG LElfFile::ReadSignedLEB128(u32 data,int *bytesRead){
   LONG result;
   int shift,count;
   u8 byte;
...
u32 LElfFile::ReadLEB128(u32 data,int *bytesRead){
   u32 result;
   int shift,count;
   u8 byte;
...
u32 LElfFile::Read4Bytes(u32 ofs){
   u32 value;

	SeekRead(ofs,&value,sizeof(u32));
   return value;
}

u16 LElfFile::Read2Bytes(u32 ofs){
   u16 value;

   SeekRead(ofs,&value,sizeof(u16));
   return value;
}

CompileUnit *LElfFile::ParseCompUnit(u32 data,u32 abbrevData){
   int bytes,i;
   u32 top,length,offset,abbrevNum;
   u16 version;
   u8 addrSize;
   ...
   data += 4;
   SeekRead(data++,&addrSize,sizeof(u8));
...
ELFAbbrev **LElfFile::ReadAbbrevs(u32 data,u32 offset){
   u8 c;
...
ELFAbbrev *LElfFile::GetAbbrev(ELFAbbrev **table,u32 number){
   ELFAbbrev *abbrev;
...
char *LElfFile::ReadString(u32 ofs){
   char c,*p;
   string s;
...
u32 LElfFile::ReadAttribute(u32 data,ELFAttr *attr){
   int bytes;
   int form;
...
start:
   switch(form) {
       ...
       case DW_FORM_block:
           ...
           data += bytes;
           attr->block->data = (u8 *)LocalAlloc(LPTR,attr->block->length+1);
           SeekRead(data,attr->block->data,attr->block->length);
           data += attr->block->length;
       break;
       case DW_FORM_block1:
           attr->block = (ELFBlock *)LocalAlloc(LPTR,sizeof(ELFBlock));
           SeekRead(data++,&attr->block->length,sizeof(u8));
           attr->block->data = (u8 *)LocalAlloc(LPTR,attr->block->length+1);
           SeekRead(data,attr->block->data,attr->block->length);
           data += attr->block->length;
       break;
       case DW_FORM_data1:
       	SeekRead(data++,&attr->value,sizeof(u8));
       break;
       case DW_FORM_flag:
           SeekRead(data++,&attr->flag,sizeof(u8));
...
u32 LElfFile::ParseCompileUnitChildren(u32 data, CompileUnit *unit){
  	int bytes;
  	u32 abbrevNum;
...
u32 LElfFile::ParseFunction(u32 data, ELFAbbrev *abbrev, CompileUnit *unit,Function **f){
  	Function *func,*fnc;
  	int bytes,i,nesting;
...
u32 LElfFile::SkipData(u32 data, ELFAbbrev *abbrev, ELFAbbrev **abbrevs){
  	int i,bytes,nesting;
	u32 abbrevNum;
...
u32 LElfFile::ParseObject(u32 data, ELFAbbrev *abbrev, CompileUnit *unit,elf_Object **object){
  	elf_Object *o;
	int i;
	ELFAttr *attr;
...
u32 LElfFile::ParseUnknownData(u32 data, ELFAbbrev *abbrev, ELFAbbrev **abbrevs){
  	int i,bytes,nesting;
	u32 abbrevNum;
...
u32 LElfFile::ParseBlock(u32 data,ELFAbbrev *abbrev,CompileUnit *unit,Function *func,elf_Object **lastVar){
  	int bytes,i,nesting;
   u32 start,end,abbrevNum;
	ELFAttr *attr;
	Function *f;
	elf_Object *o;
...
Type* LElfFile::ParseType(CompileUnit *unit,u32 offset){
	Type *t,*type;
   u32 data;
	int bytes,abbrevNum;
	ELFAbbrev *abbrev;
...
void LElfFile::ParseType(u32 data,u32 offset,ELFAbbrev *abbrev,CompileUnit *unit,Type **type){
	u32 typeref,num;
   char *name;
...
void LElfFile::GetObjectAttributes(CompileUnit *unit,u32 offset, elf_Object *o){
  	u32 data,abbrevNum;
  	int bytes,i;
	ELFAbbrev *abbrev;
   ELFAttr *attr;
...
void LElfFile::AddLine(LineInfo *l,u32 a, int file, int line, int *max){
   LineInfoItem *li;
...
CompileUnit *LElfFile::GetCompileUnitForData(u32 data){
  	u32 end;
	CompileUnit *unit;
...
void LElfFile::ParseAranges(u32 data){
  	ELFSectionHeader *sh;
	u32 end,len,offset,addr;
	int max,index,i;
   ARanges *ranges;

  	sh = GetSectionByName((char *)".debug_aranges");
  	if(sh == NULL)
    	return;
...
void LElfFile::ParseLocations(u32 top){
  	ELFSectionHeader *h;
   u32 data,end,lowPC,highPC,offset,count;
   u16 len;
   Function *f;
   elf_Object *o;
   int i1,i;
   ...
   h = GetSectionByName((char *)".debug_loc");
  	if(h == NULL)
    	return;
   ...
   while(data < end){
       ...
       Locations[count].block->data = (u8 *)(Locations[count].block + 1);
   ...
   for(i=0;i<LocationsCount;i++){
       ...
       while(comp != NULL){
           f = comp->functions;
           while(f != NULL){
               if(f->frameBase == (ELFBlock *)(u64)Locations[i].offset){
               ...
               o = f->variables;
               while(o != NULL){
                   if(o->location == (ELFBlock *)(u64)Locations[i].offset){
                       for(i1=i;i < LocationsCount && Locations[i].offset == Locations[i1].offset;i++){
                           Locations[i].lowPC += comp->lowPC;
                           Locations[i].highPC += comp->lowPC;
                       }
...
void LElfFile::ParseCFA(u32 top){
  	ELFSectionHeader *h;
   u32 data,topOffset,end,offset,len,dataEnd,id;
   ELFcie *cies,*cie;
	int bytes;
	ELFfde *fde;

   h = GetSectionByName((char *)".debug_frame");
  	if(h == NULL)
    	return;
  	data = ReadSection(h,top);
  	topOffset = data;
  	...
  	while(data < end) {
   	offset = data - topOffset;
    	...
    	if(id == 0xffffffff){
           ...
           cies = cie;
      		cie->offset = offset;
      		if((cie->augmentation = (u8 *)ReadString(data++)) != NULL)
           ...
           cie->dataLen = dataEnd - data;
      		cie->data = (u8 *)LocalAlloc(LPTR,cie->dataLen);
...
//        		exit(-1);
           ...
           fde->data = (u8 *)LocalAlloc(LPTR,fde->dataLen);
...
void LElfFile::ReadSymtab(u32 data){
	...
	u32 top;
  	...
  	sh = GetSectionByName((char *)".symtab");
   if(sh == NULL)
   	return;
  	table = sh->link;
...
CompileUnit *LElfFile::GetCompileUnit(u32 addr){
	CompileUnit *unit;
	ARanges *r;
	int count,j;
...
int LElfFile::GetVariableFromName(const char *name,u32 adr,void **obj,void **sym,void **u){
   Symbol *s;
   elf_Object *v;
   int i,res;
  	CompileUnit *unit;
  		        ...
  		        if(adr >= func->lowPC && adr < func->highPC){
                   ...
                   while(v){
                       if(lstrcmpi(name,v->name) == 0){
                           ...
                           *sym = v->location;
                           for(i=0;i<LocationsCount;i++){
                               if(v->location == (ELFBlock *)(u64)Locations[i].offset){
...
int LElfFile::GetCurrentFunction(u32 addr,void **f,void **u,void **l){
  	CompileUnit *unit;
	Function *func;
  	int count,i;
	LineInfoItem *table;
...
int LElfFile::_getInfo(u32 *psz,void **o){
	u32 *p,sz;
	int i;

	if(!psz) return -2;
	sz=header.e_phnum*sizeof(u32)*4+sizeof(u32);
	//if(o) *o=0;
	if(o && *o && *psz<sz) {*psz=sz;return -2;}
	*psz=sz;
	if(!o)
		goto Z;
	if(ProgramHeader == NULL)
		return -3;
	p=(u32*)*o;
	if(!p && !(p=new u32 [sz/sizeof(u32)]))
		return -4;
	*o=p;
	*p++=header.e_entry;
	for(i=0;i<header.e_phnum;i++){
		*p++=ProgramHeader[i].offset;
		*p++=ProgramHeader[i].paddr;
		*p++=ProgramHeader[i].filesz;
		p++;
	}
Z:
	return 0;
           ...
           if((ProgramHeader[i].paddr & 0x0F000000) == 0x02000000){
              /// mem = ext_mem;
               ...
               if(ProgramHeader[i].paddr & 0x00800000){
                   adr = (u32)(u16)ProgramHeader[i].paddr;
              //     mem = int_mem2;
               }
               else{
...
HLOCAL LElfFile::LocalReAlloc(HLOCAL hMem,u32 uBytes,u32 uFlags){
#ifdef __WIN32__
	HLOCAL mem;
	u32 uSize;
