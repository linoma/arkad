...
switch(mm##M(mode)){\
	...
	case 4:{c__=&REG_A(mm##R(mode));mm##DEC##am(sz,*(u32 *)c__);__address=*(u32 *)c__;RMAP_PC(__address,d__,R);m##M##am(sz,__address,v__);} goto CONCAT(A,__LINE__);break;\
	...
	case 6:{RWPC(_pc+iop,v__);iop+=2;u32 v0__=REG_D(SR((u16)v__,12));v__=REG_A(mm##R(mode)) + (s8)v__ + (s32)((v__&0x800) ? v0__ : (s16)v0__);RMAP_PC(v__,d__,R);__address=v__;m##M##am(sz,__address,v__);goto CONCAT(A,__LINE__);}break;\
	case 7:\
		switch(mm##R(mode)){\
			case 0:RWPC(_pc+iop,v__);iop += 2;__address=v__;m##M##am(sz,v__,v__);RMAP_PC(__address,d__,R);goto CONCAT(A,__LINE__);break;\
			case 1:RLOP(_pc+iop,v__);iop += 4;__address=v__;m##M##am(sz,v__,v__);RMAP_PC(__address,d__,R);goto CONCAT(A,__LINE__);break;\
			case 2:RWPC(_pc+iop,v__);v__=_pc+ipc + (s16)v__;__address=v__;iop += 2;m##M##am(sz,v__,v__); RMAP_PC(__address,d__,R);goto CONCAT(A,__LINE__);break;\
			case 3:{RWPC(_pc+iop,v__);u32 v0__=REG_D(SR((u16)v__,12));v__=_pc+ipc + (s8)v__+ (s32)((v__&0x800) ? v0__ : (s16)v0__);iop += 2;RMAP_PC(v__,d__,R);__address=v__;m##M##am((sz),__address,v__); goto CONCAT(A,__LINE__);}break;\
...
#define BIT_INSI(ins){BIT_INS_(ins,RWPC(_pc+ipc,b);ipc+=2;sz=-1;);\
...
int M68000Cpu::Init(void *m,u32 ss,u32 f){
	...
	for(int n=0;n<0x8;n++){
		for(int i=0;i<0x8;i++){
			...
			M68OPCODE(((0x1000|SL(n,6)|0x0|i)*2),&M68000Cpu::_or_reg);
			M68OPCODE(((0x1000|SL(n,6)|0x8|i)*2),&M68000Cpu::_or_reg);
			M68OPCODE(((0x1000|SL(n,6)|0x10|i)*2),&M68000Cpu::_or_reg);
			M68OPCODE(((0x1000|SL(n,6)|0x20|i)*2),&M68000Cpu::_or_reg);
			M68OPCODE(((0x1000|SL(n,6)|0x28|i)*2),&M68000Cpu::_or_reg);
			M68OPCODE(((0x1000|SL(n,6)|0x30|i)*2),&M68000Cpu::_or_reg);
			...
			M68OPCODE(((0x1800|SL(n,6)|0x0|i)*2),&M68000Cpu::_and_reg);
			M68OPCODE(((0x1800|SL(n,6)|0x8|i)*2),&M68000Cpu::_and_reg);
			M68OPCODE(((0x1800|SL(n,6)|0x10|i)*2),&M68000Cpu::_and_reg);
			M68OPCODE(((0x1800|SL(n,6)|0x20|i)*2),&M68000Cpu::_and_reg);
			M68OPCODE(((0x1800|SL(n,6)|0x28|i)*2),&M68000Cpu::_and_reg);
			M68OPCODE(((0x1800|SL(n,6)|0x30|i)*2),&M68000Cpu::_and_reg);
...
int M68000Cpu::_exec(u32 status){
	...
	*/
	void *tmp__;
	RMAP_PC(_pc,tmp__,R);
	__data=tmp__ ? *((u64 *)tmp__) : 0;
	*(u64 *)&_icache._data[_icache._idx * 8]=__data;
	_opcode=SWAP16((u16)__data);
...
Z:
	if(_icache._invalidate || ipc > 5){
		_icache.p=0;
		_icache._invalidate=0;
	}
	else
		_icache.p=&_icache._data[_icache._idx++ * 8 + ipc];
	return ret;
}
